...
class WTE_Booking_Emails extends WP_Travel_Engine_Emails {
	...
	private function header( $sendto ) {
		$customer_info = $this->booking->billing_info;
		$customer_name = $customer_info['fname'] .' '. $customer_info['lname'];
		$admin_from_receipt = "{$customer_name} < {$customer_info['email']} >";
...
<?php
	...
	<table width="100%" cellpadding="0" cellspacing="0">
		...
		<tr>
			...
			<td class="alignright">
				<?php echo $trip->has_time ? wp_date( 'Y-m-d H:i', strtotime( $trip->datetime ) ) : wp_date( get_option( 'date-format', 'Y-m-d' ), strtotime( $trip->datetime ) ); ?>
			</td>
		</tr>
		<tr>
			<td>
	...
	<?php
			...
			<table width="100%">
				...
				<?php if ( ! empty( $cart_info['tax_amount'] ) ) { ?>
					<tr>
						<td>
							<?php echo wptravelengine_get_tax_label( $cart_info['tax_amount'] ); ?>
						</td>
						<?php
						$tax_figure       = 0;
...
<?php
	...
	public function send() {
		...
		if ( 'admin' === $this->sendto && isset( $global_settings['email']['disable_notif'] ) && '1' === $global_settings['email']['disable_notif'] ) {
			return;
		}
		wp_mail( $this->to, $this->subject( $this->email_template_type ), is_null( $this->content ) ? $this->template( $this->email_template_type )->content : $this->content, $this->header( $this->sendto ), $this->get_attachments() );
	}
