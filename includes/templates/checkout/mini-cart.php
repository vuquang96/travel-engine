...
if ( ! empty( $cart_items ) ) :
					...
					<tbody>
								...
								?>
									<tr class="wte-coupons-discount-calculation-tr">
										...
										<td class="wte-coupons-discount-calculation-td">
											<b>
										<?php
										if ( isset( $cart_data['discountamount'] ) ) {
											echo '-' . wte_get_formated_price_html( $cart_data['discountamount'], null, true );
										}
										if ( isset( $cart_data['discountpercent'] ) ) {
											echo '-' . wte_get_formated_price_html( $cart_data['discountpercent'], null, true );
										}
										?>
											</b>
										</td>
						...
						if ( $has_tax ) {
							...
							<tr class="wte-tax-calculation-tr">
								<td>
									<span><?php echo esc_html( wptravelengine_get_tax_label( (int) $cart_totals['tax_amount'] ) ); ?></span>
								</td>
								<td>
						...
						<td><span><?php esc_html_e( 'Remaining Payment', 'wp-travel-engine' ); ?></span></td>
							...
							if ( ( ! empty( $cart_discounts ) || sizeof( $cart_discounts ) !== 0 ) ) {
								if ( $cart_totals['tax_amount'] != '' ) {
									$partial_final_total = ( $cart_data['totalcost'] - (float) $cart_totals['total_partial'] );
								} else {
									$partial_final_total = ( intval( $cart_data['cost'] ) - (float) $cart_totals['total_partial'] );
								}
	...
	<?php
	$payable_amount = wptravelengine_get_payable_amount( $cart_discounts, $tripid, $cart_totals, $cart_data['cost'], $cart_data['totalcost'] );
	$tax_enable     = isset( $wte_settings['tax_enable'] ) ? $wte_settings['tax_enable'] : 'no';
	?>
		<div class="wpte-bf-total-price">
			...
			<span class="wpte-price-wrap">
				<?php echo wte_esc_price( wte_get_formated_price_html( $payable_amount, null, true ) ); ?>
			</span>
			<?php
			if ( $tax_enable == 'yes' && isset( $wte_settings['tax_type_option'] ) && 'inclusive' === $wte_settings['tax_type_option'] ) {
				$tax_percentage = $wte_settings['tax_percentage'];
				printf( '<span class="wpte-inclusive-tax-label">%s</span>', sprintf( __( '(%s%% Incl. tax)', 'wp-travel-engine' ), $tax_percentage ) );
			}
			?>
		</div>
