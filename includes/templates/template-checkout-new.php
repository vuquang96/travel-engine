			...
			<div class="wpte-bf-checkout-form">
				...
				<form id="wp-travel-engine-new-checkout-form" method="POST" name="wp_travel_engine_new_checkout_form" action="" enctype="multipart/form-data"
					...
					if ( wp_travel_engine_is_cart_partially_payable() ) :
						...
						$global_full_payment  = isset( $partial_payment_data['global_full_payment'] ) && $partial_payment_data['global_full_payment'];
						$trip_full_payment    = isset( $partial_payment_data['trip_full_payment'] ) && $partial_payment_data['trip_full_payment'];
						$trip_actual_price    = wte_get_formated_price_html( wptravelengine_get_payable_amount( $cart_discounts, $tripid, $cart_totals, $cart_data['cost'], $cart_data['totalcost'] ), null, true );

						if ( ! empty( $partial_payment_data ) ) :
							if ( 'amount' === $partial_payment_data['type'] ) :
								$trip_price_partial = wte_get_formated_price( $partial_payment_data['value'] );
								$trip_price_full    = $trip_actual_price;
					...
					<div class="wpte-bf-field wpte-bf-radio wpte-bf_downpayment-options">
						 ...
						 * @since 5.7.1
						 */
						if ( $global_full_payment && $trip_full_payment ) :
							?>
							...
							<div class="wpte-bf-radio-wrap">
								...
								<label for="wp_travel_engine_payment_mode-full">
									<?php
									echo sprintf( esc_html( $full_payment_label ), wte_esc_price( $trip_price_full ) );
									?>
								</label>
							</div>
						<?php endif; ?>
					...
					<div class="wpte-bf-field wpte-bf-radio wpte-bf_payment-methods">
						...
						foreach ( $active_payment_methods as $key => $payment_method ) :
							...
							<div class="wpte-bf-radio-wrap wpte-bf_payment-method">
								<input data-target-info="wpte__checkout-info--<?php echo esc_attr( $key ); ?>" <?php checked( $default_gateway, true ); ?> type="radio" name="wpte_checkout_paymnet_method" value="<?php echo esc_attr( $key ); ?>" id="wpte-checkout-paymnet-method-<?php echo esc_attr( $key ); ?>">
					...
					<div class="wpte-bf-field wpte-bf-submit">
						...
						value="<?php echo esc_attr( wte_default_labels( 'checkout.submitButtonText' ), 'wp-travel-engine' ); ?>">
					</div>
