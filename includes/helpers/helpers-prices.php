...
function wptravelengine_get_payable_amount( $cart_discounts, $trip_id, $cart_totals, $new_tcost, $new_totalcost ) {
	$wp_travel_engine_settings = get_option( 'wp_travel_engine_settings', true );
	if ( ( ! empty( $cart_discounts ) || sizeof( $cart_discounts ) !== 0 ) && wp_travel_engine_is_trip_partially_payable( $trip_id ) ) {
		$payable_now = $cart_totals['total_partial'];
	} elseif ( ( ! empty( $cart_discounts ) || sizeof( $cart_discounts ) !== 0 ) && ! wp_travel_engine_is_trip_partially_payable( $trip_id ) ) {
...
/**
 * Retrieve necessary data to be displayed in checkout page.
 *
 * @since 5.7.1
 *
 * @param mixed $cart_discounts Cart Discount Amount.
 * @param mixed $trip_total Cart Total Amount.
 * @param mixed $cart_totals Cart Total.
 */
function wptravelengine_get_newcost( $cart_discounts, $trip_total, $cart_totals ) {
	$calculated_total       = isset( $trip_total['cart_total'] ) && ! empty( $trip_total['cart_total'] ) ? intval( $trip_total['cart_total'] ) : 0;
	$cart_data              = array();
	$new_tcost              = 0;
	$new_totalcost          = 0;
	$discount_amount        = 0;
	$discount_amount_actual = 0;
	$tax_actual             = 0;
	foreach ( $cart_discounts as $discount_key => $discount_item ) {
		if ( 'fixed' === $discount_item['type'] ) {
			...
			$new_totalcost   = number_format( ( $new_tcost + $tax_actual ), '2', '.', '' );
			$discount_amount = $discount_item ['value'];
	...
	$cart_data = array(
		...
		'taxactual'       => $tax_actual,
		'discountamount'  => $discount_amount,
		'discountpercent' => $discount_amount_actual,
	);
	return $cart_data;
